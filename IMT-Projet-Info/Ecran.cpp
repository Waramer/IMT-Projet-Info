...
void Ecran::affiche() {
    ...
    //définition/initialisation des variables et constantes
    ...
    while (!WindowShouldClose())
    {
        ...
        */
        while (res == 0) {
            if (GetSoundsPlaying() == 0 && musicON != 1)    //on lance la musique si elle n'est pas désactivée dans les réglages
                PlaySoundMulti(sound);
            ...
            //changement d'écran selon le menu activé
        ...
        /*
            -------------------------------------------------------------------------Ecran de Jeu --------------------------------------------------------------------------------------------------
        */
        if (res == 0) { break; }  //on quitte le jeu si l'utilisateur a sélectionné cette option
        ...
        while (res == 1) {
            ...
            if (GetSoundsPlaying() == 0 && musicON != 1) //on lance la musique du jeu
                PlaySoundMulti(sound2);
            ...
            if (j.collisionCurseur(pointeur.x, pointeur.y, angle) == true) {
                ...
                //Ouvrir le menu game over
                StopSoundMulti();
                PlaySoundMulti(sound5);
                res = 5;
            }

            if (IsKeyDown(KEY_P)) {   //ouvrir le menu pause
                EndDrawing();
                res = 2;
            }
            ...
            */
            ...
            while (res == 2) {
                ...
                //déplacement entre les différentes sélections avec les touches up et down
        ...
        */
        ...
        while (res == 3) {
            // Page Highscores qui affiche les meilleurs scores pour chaque niveau de difficulté
            BeginDrawing();
            ClearBackground(color);

            DrawText(("%c", s2), x / 2 - tailleS2 / 2, 100, 120, WHITE);

            DrawText("EASY : ", x / 4, 300, 40, bronze);
            for (int i = 0; i < 5;i++) {
                DrawText(TextFormat("#%d : %d", i + 1, scoresTabEasy[i]), x / 4, 400 + 100 * i, 40, DARKGRAY);
            }

            DrawText("MEDIUM : ", x / 2, 300, 40, LIGHTGRAY);
            for (int i = 0; i < 5;i++) {
                DrawText(TextFormat("#%d : %d", i + 1, scoresTabMedium[i]), x / 2, 400 + 100 * i, 40, DARKGRAY);
            }

            DrawText("HARD : ", 3 * x / 4, 300, 40, GOLD);
            for (int i = 0; i < 5;i++) {
                DrawText(TextFormat("#%d : %d", i + 1, scoresTabHard[i]), 3 * x / 4, 400 + 100 * i, 40, DARKGRAY);
            }

            DrawText(("c", s3), x / 2 - tailleS3 / 2, y - 100, 40, RED);
            if (IsKeyPressed(KEY_ENTER)) {  //retour au menu principal
                ClearBackground(BLACK);
                res = 0;
            }
        ...
        */
        ...
        while (res == 5) {
            ...
            if (IsKeyPressed(KEY_ENTER)) {
                if (selectionGameOver == 0) {   //relancer une partie
                    res = 1;
                    pointeur.x = x * 0.5;
                    pointeur.y = y * 0.5;
                    StopSoundMulti();
